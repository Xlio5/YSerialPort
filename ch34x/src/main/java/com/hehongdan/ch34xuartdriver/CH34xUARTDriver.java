...
public class CH34xUARTDriver {
    ...
    public CH34xUARTDriver(UsbManager usbManager, Context context, String filter) {
        ...
        this.addSupportVendorProduct("1a86:55e3");
    }
    ...
    private void ensureReceiverRegistered() {
        if (this.isRegisterReceiver) return;
        IntentFilter filter = new IntentFilter(this.broadcastReceiverFilter);
        filter.addAction("android.hardware.usb.action.USB_DEVICE_DETACHED");
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            this.context.registerReceiver(this.ch34BroadcastReceiver, filter, Context.RECEIVER_NOT_EXPORTED);
        } else {
            this.context.registerReceiver(this.ch34BroadcastReceiver, filter);
        }
    ...
    /**
     * 获取当前连接的所有支持的CH34x设备列表
     * @return 匹配的设备列表
     */
    public List<UsbDevice> enumerateDeviceList() {
        ...
        if (devices != null && !devices.isEmpty()) {
            for (UsbDevice device : devices.values()) {
                for (int i = 0; i < this.supportTypeSize; i++) {
                    if (String.format("%04x:%04x", device.getVendorId(), device.getProductId())
                            .equals(this.supportVendorProduct.get(i))) {
